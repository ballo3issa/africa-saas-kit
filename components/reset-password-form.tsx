...
export function ResetPasswordForm(){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();if(!token)return;const password=String(new FormData(e.currentTarget).get("password")||"");const r=await authClient.resetPassword({newPassword:password,token});if(r.error){setMessage(r.error.message||"Lien invalide ou expiré");return;}setMessage("Mot de passe modifié. Vous pouvez vous reconnecter.");setTimeout(()=>router.push("/login"),700)}
