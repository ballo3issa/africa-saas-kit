...
| 18 | Connexion | `/login` | **ADAPTER** | `app/(auth)/login/page.tsx` |

## Features existantes potentiellement concernées
- **auth** — Better Auth, rôles, organisations, 2FA
- **email** — Emails transactionnels Resend; requis si email/password + vérification/récupération sont activés
- **google** — Google OAuth + Search Console
...
- Composants candidats à réutiliser: `components/._landing-page.tsx`, `components/._studeo-page-skeleton.tsx`, `components/landing-page.tsx`, `components/studeo-page-skeleton.tsx`, `components/._landing-header.tsx`
...
### 18. Connexion — ADAPTER
- Route Banani: `/login`
- Existant: `app/(auth)/login/page.tsx`
- Décision: La route existe déjà (app/(auth)/login/page.tsx) ; préserver la logique et adapter le rendu au design.
- Composants candidats à réutiliser: aucun candidat évident
- Auth observée: public
- Données visibles: Adresse email, Mot de passe, Session utilisateur
- Interactions visibles: Se connecter avec Google, Se connecter avec email et mot de passe, Afficher ou masquer le mot de passe, Ouvrir l'inscription, Ouvrir la récupération de mot de passe, Ouvrir les CGU ou la Politique de confidentialité
- Intégrations visibles/confirmées: Google OAuth, Better Auth
