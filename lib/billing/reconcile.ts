...
import { db } from "@/db";
import { paymentFulfillments, payments, plans, subscriptions } from "@/db/schema";
...
const currencyDecimals: Record<string, number> = { XOF: 0, XAF: 0, NGN: 2, GHS: 2, KES: 2, USD: 2, EUR: 2 };
...
function paidDate(raw: any, fallback: Date) {
  const data = raw?.data ?? raw;
  const candidates = [data?.settled_at, data?.paid_at, data?.completed_at, data?.updated_at, data?.created_at, data?.created_datetime];
...
function addCalendarPeriod(date: Date, interval: "month" | "year") {
  ...
  return new Date(Date.UTC(targetYear, targetMonth, Math.min(day, lastDay), date.getUTCHours(), date.getUTCMinutes(), date.getUTCSeconds(), date.getUTCMilliseconds()));
}
...
export async function reconcilePayment(paymentId: string) {
  ...
  if (!payment.providerPaymentId) return { paymentId, status: payment.status, skipped: "missing-provider-id" };
  ...
  if (verified.status === "paid") {
    ...
    await db.insert(paymentFulfillments).values({
      id: randomUUID(), paymentId: payment.id, provider: provider.id, appliedAt: new Date(),
    }).onConflictDoNothing({ target: paymentFulfillments.paymentId });
  } else if (verified.status === "failed" && payment.status !== "paid") {
    await db.update(payments).set({ status: "failed" }).where(eq(payments.id, payment.id));
  }

  return { paymentId: payment.id, provider: provider.id, status: verified.status };
}
