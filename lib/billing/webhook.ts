...
function eventReference(payload: any): string | undefined {
  ...
  return metadata?.app_reference ?? data?.tx_ref ?? data?.reference ?? data?.merchant_reference ?? data?.ref_command ?? data?.paymentReference;
}
...
export async function processPaymentWebhook(provider: PaymentProvider, event: { id: string; type: string; payload: unknown }) {
  ...
  const [stored] = await db.select().from(webhookEvents)
    .where(and(eq(webhookEvents.provider, provider.id), eq(webhookEvents.externalEventId, event.id))).limit(1);
  if (!stored || stored.processed) return { duplicate: true };

  let payment;
  if (externalId) {
    [payment] = await db.select().from(payments)
      .where(and(eq(payments.provider, provider.id), eq(payments.providerPaymentId, externalId))).limit(1);
  }
  if (!payment && reference) {
    [payment] = await db.select().from(payments)
      .where(and(eq(payments.provider, provider.id), eq(payments.reference, String(reference)))).limit(1);
  }
  ...
  if (externalId && payment.providerPaymentId !== externalId) {
    ...
    await db.update(payments)
      ...
      .where(and(eq(payments.id, payment.id), or(isNull(payments.providerPaymentId), eq(payments.providerPaymentId, payment.reference))));
