...
const rows = await sql`
  ...
  WHERE n.nspname = 'public' AND c.relkind = 'r' AND c.relname = ANY(${required})
`;
...
console.log(`Security DB check: PASS — RLS and policies verified on ${required.length} required tables.`);
