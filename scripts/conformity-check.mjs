...
const requiredFiles = [
  ...
  ".agents/skills/setup-saas/SKILL.md", "docs/setup-saas.md", ".agents/skills/computer-use/SKILL.md", "docs/computer-use/antigravity-browser.md", "scripts/computer-use-check.mjs", "scripts/computer-use-mark.mjs",
...
if (exists("package.json")) {
  ...
  const expectedScripts = [
    ...
    "mobile:check", "ui:icons-check", "ui:loading-check", "seo:check", "deploy:check", "doctor:production",
    "doctor:production:online", "conformity:check", "provider", "typecheck", "build",
    "lint", "format:check", "test", "audit:prod", "routes:list", "runtime:check", "features:list", "features:check", "smoke:system", "cron:generate", "computer-use:check", "computer-use:mark", "banani:prepare", "banani:check", "import-banani:check", "import-banani:analyze"
  ];
...
if (exists(".agents/skills/setup-saas/SKILL.md")) {
  ...
  for (const phrase of ["20 phases", "Computer Use", "Upstash", "Paiements", "Cloudflare", "Cloudinary", "conformity:check", "banani:prepare", ".codex/config.toml"]) {
