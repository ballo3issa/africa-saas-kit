...
if (!setupSaasText.includes("Phase 15") && !setupSaasText.includes("paiements (OPTIONNEL)")) warnings.push("Review optional payment phase wording");
const cloudflareSetup = fs.readFileSync("scripts/setup-cloudflare.mjs", "utf8");
if (!cloudflareSetup.includes("--none") || !cloudflareSetup.includes("phase=16")) errors.push("Optional Cloudflare setup/skip flow missing");
const workflowText = fs.readFileSync(".agents/skills/setup-saas/SKILL.md", "utf8");
if (!workflowText.includes("18 phases") || !workflowText.includes("Cloudflare domaine/DNS") || !workflowText.includes("Cloudinary") || !workflowText.includes("conformity:check")) errors.push("/setup-saas roadmap must include 18 phases, optional services and final conformity check");
...
console.log("Security preflight: V0.8.16 checks passed.");
